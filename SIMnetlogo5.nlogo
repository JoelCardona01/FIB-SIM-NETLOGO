    ...
    (ifelse
      (ncaçadors + nnormals + nexploradors) >= 3 [
        ask humans with [xhome = cordenadax and yhome = cordenaday] [
          if ne = 0 [
            ifelse nn > 2 or nc > 2 and rol > 10 and rol <= 50  [set rol 5 set color black show "he creat un negre1" set ne ne + 1 set nn nn - 1]
            [if nc > 0 and rol > 50 [set rol 5 set color black show "he creat un negre2" set ne ne + 1 set nc nc - 1]]
          ]
          if nc = 0 [
            if nn > 2 or ne > 2 and rol > 10 and rol <= 50  [set rol 60 set color yellow show "he creat un marró" set nc nc + 1 set nn nn - 1]
          ]
          if nn < 2 [
...
to reproduceHumans [min-age birth-rate]
   if age >= min-age and nreproductions < human-max-reproduction and reproduceticks = 0 [
    ...
    hatch 1 [
      ...
      set nreproductions 0
      set reproduceticks 0
      set rol random 100 ; Si es de 0 a 0.2 explorador. De 0.21 a 0.5 normal. Caçador 0.51 a 1
           ifelse rol <= 10
          [ set color black ask patch xhome yhome [set nexploradors nexploradors + 1] ]
...
to die-naturally-human
  if age >= human-max-age [
    ...
    if hasHome xhome yhome [ask patch xhome yhome [
      ifelse rolturtle <= 10
          [set nexploradors nexploradors - 1]
          [ ifelse rolturtle > 10 and rolturtle <= 50
            [ set nnormals nnormals - 1]
