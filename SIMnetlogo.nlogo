...
to SETUP
  ...
  create-mammoths 10 [
   ...
   create-humans 10 [
    ...
    set age random (25 * 365)
    set nreproductions 0
    set reproduceticks 0
    set xhome max-pxcor + 1
    set yhome max-pycor + 1
    move-to one-of patches
    set rol random (1) ; Si es de 0 a 0.2 explorador. De 0.21 a 0.5 normal. Caçador 0.51 a 1
  ]
reset-ticks
end
  ...
  ask humans [
    ...
    if any? other humans-here and xhome = (max-pxcor + 1) and yhome = (max-pycor + 1)[
      ...
      ifelse hasHome [xhome] of h [yhome] of h
        [
         ...
         ask patch xhome yhome [set nhumans nhumans + 1]
        ]
        [
         set xhome xcor
         set yhome ycor
         set pcolor red
         ask patch-here [set nhumans nhumans + 1]
      ]
    ]
    if any? other humans-here and hasHome xhome yhome [
      ...
      if hasHome [xhome] of h [yhome] of h and patch xhome yhome != patch [xhome] of h [yhome] of h [
        ...
        ask patch xhome yhome [set numHumansHome1 nhumans]
        let numHumansHome2 0
        ask patch [xhome] of h [yhome] of h [set numHumansHome2 nhumans]
...
to reproduceHumans [min-age birth-rate]
   if age >= min-age and nreproductions < human-max-reproduction and reproduceticks = 0 [
    ...
    hatch 1 [set age 0 set xhome [xhome] of myself set yhome [yhome] of myself set nreproductions 0 set reproduceticks 0 move human-speed]
    ask patch xhome yhome [set nhumans nhumans + 1]
    show "hola he chingao"
   ]
...
to die-naturally-human
  if age >= human-max-age [
    if hasHome xhome yhome [ask patch xhome yhome [set nhumans nhumans - 1]]
    die
  ]
end
@#$#@#$#@
